...
abstract class AppDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        private val MIGRATION_9_10 = object : Migration(9, 10) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE `agent_runs` ADD COLUMN `plan` TEXT NOT NULL DEFAULT ''")
                db.execSQL("ALTER TABLE `agent_runs` ADD COLUMN `currentStep` INTEGER NOT NULL DEFAULT 0")
                db.execSQL("ALTER TABLE `agent_runs` ADD COLUMN `maxSteps` INTEGER NOT NULL DEFAULT 6")
                db.execSQL("ALTER TABLE `agent_runs` ADD COLUMN `recoveryState` TEXT NOT NULL DEFAULT ''")
            }
        }
